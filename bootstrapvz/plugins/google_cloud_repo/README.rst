...
-  ``cleanup_bootstrap_key``: Deletes the bootstrap key (``/etc/apt/trusted.gpg.d/google-cloud-bootstrap.asc`` or ``.gpg``) in favor of the package maintained version. This is only to avoid having multiple copies of the key trusted by apt. This should only be used with enable_keyring_repo.
