...
for readme_path in glob.glob('../bootstrapvz/providers/*/README.rst'):
    ...
    if not os.path.exists(include_path):
        ...
        with open(include_path, 'w', encoding='utf-8') as include:
...
import subprocess
try:
    git_head = subprocess.check_output(['git', 'describe', '--tags'], stderr=subprocess.DEVNULL,
                                       universal_newlines=True).strip()
except (OSError, subprocess.CalledProcessError):
    try:
        git_head = subprocess.check_output(['git', 'rev-parse', 'HEAD'], stderr=subprocess.DEVNULL,
                                           universal_newlines=True).strip()[:10]
    except (OSError, subprocess.CalledProcessError):
        git_head = 'master'
